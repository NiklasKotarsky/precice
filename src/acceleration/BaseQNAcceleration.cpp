...
void BaseQNAcceleration::initializeVectorsAndPreconditioner(const DataMap &cplData)
{
  ...
  auto         addTimeSliceSize = [&](size_t sum, int id) { return sum + _timeGrids.getTimeGrid(id).size() * cplData.at(id)->getSize(); };
  const size_t primaryDataSize  = std::accumulate(_primaryDataIDs.begin(), _primaryDataIDs.end(), (size_t) 0, addTimeSliceSize);
  const size_t dataSize         = std::accumulate(_dataIDs.begin(), _dataIDs.end(), (size_t) 0, addTimeSliceSize);
  ...
  PRECICE_ASSERT(_primaryValues.size() == 0, "Values already initialized, something is wrong with the QN method");
  PRECICE_ASSERT(_oldPrimaryValues.size() == 0, "Values already initialized, something is wrong with the QN method");
